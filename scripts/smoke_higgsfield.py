"""Smoke test: generate scene 0 animation with Higgsfield DoP.

Usage:
    python scripts/smoke_higgsfield.py              # defaults to project 'redbull_sleep'
    python scripts/smoke_higgsfield.py <project>    # uses output/<project>/state.json

Reads an existing state.json (so the project must already have scene 0's
first/last frames generated), forces the Higgsfield provider regardless
of VIDEO_PROVIDER, and writes the clip to
output/<project>/videos_smoke_higgsfield/.
"""
import sys
import time
...
from src.providers.higgsfield import HiggsfieldProvider  # noqa: E402
...
def main() -> None:
    ...
    print(f"[Smoke] Tier: {config.higgsfield_tier}")
    print()

    vg = HiggsfieldProvider(config)
    print(f"[Smoke] {vg.describe()}")
    out_dir = config.output_dir / project_name / "videos_smoke_higgsfield"
    t0 = time.time()
